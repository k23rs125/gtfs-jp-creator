...
> **詳しい手順（準備＋利用の流れ＋トラブルシュート）→ [docs/利用手順.md](docs/利用手順.md)**
> 人間向けの俯瞰は [使い方・統合ガイド](使い方_統合ガイド.md)、
> 試用してフィードバックをくれる方は [docs/試用ガイド.md](docs/試用ガイド.md)（[記入テンプレ](docs/フィードバック記入テンプレ.md)）を参照。
...
主な特徴:
- **半自動・ask-first**：PDF/Excel に無い情報（事業者・運行日・**運賃〔大人/小児/障がい者の区分別可〕**など）は
  推測せず一括質問する。不明分は公式オープンデータ（BODIK 等）も能動確認し、暫定値には「要確認」を明記。
- **画像化PDFのOCR**：MinerU を **pipeline バックエンド既定**（数字に正確・1ページ数分）で実行し、
  Markdown を `extract_timetable_markdown.py` で取り込む。**多路線（往復＝同一停留所集合）は自動でまとめる**。
- **座標補完の多層化**：旧/公式フィード再利用(3.5a) → P11(3.5b) → 同名複数候補の経路位置選択(3.5b2)
  → Nominatim(3.5c) → 手動(3.5d) → 経路ジオメトリ外れ値棄却(3.5d2) → 内挿推定(3.5e)
  → **信頼度分類(3.5f)**。**第3相＝独立な複数ソースが近接一致したら確定に昇格**し要確認を削減。
  **公式GTFSの座標自動再利用**で POI 主体路線も対公式 中央値 **0m** を達成。
- **ダイヤ・運賃・運休日の多様化**：平日/土日で時刻が違う路線は**複数カレンダー**に分離、
  運賃は**均一(大人/小児/障がい者の区分別)／区間制(zone)**、**祝日・年末年始・お盆の運休**を
  `calendar_dates` に展開（祝日は内閣府CSV同梱）。
- **検証＋目視支援**：MobilityData Validator(7) ＋ GTFS-JP 拡張検証(7b) ＋ **内部整合検証(7c)**
  ＋ **時刻アノマリ検出(7d)**（OCR誤読の疑いを便間パターンで検出）。
  **GTFSビューア(gtfs_viewer.html)** を生成し、路線図/時刻表/運賃表など7タブをブラウザ確認。
- **「正しく失敗」**：誤った座標・時刻・メタは作らず、未確定として要確認に回す（官公庁提出の核）。
- **画面操作版（Streamlit アプリ）**：抽出→路線割り当て→全時刻の目視確認→条件入力→生成→
  **地図でピンをドラッグして座標確定**→7タブビューア。`streamlit run app/app.py`（[app/README](app/README.md)）。
...
run_pipeline.py --config <config.json>
  ...
  ├ Step 3.5a 旧/公式フィードから座標再利用   （reference/official_feed 指定時）
  ├ Step 3.5b 国土数値情報 P11 で座標補完    （p11_prefecture 指定時・自動DL）
  ├ Step 3.5b2 同名複数候補の経路位置選択
  ├ Step 3.5c Nominatim で座標補完          （use_nominatim=true 時）
  ├ Step 3.5d2 経路ジオメトリ外れ値棄却
  ├ Step 3.5e 経路内挿で座標推定
  ├ Step 3.5f 座標の信頼度分類（確定/要確認/未補完・第3相＝複数ソース一致で確定昇格）
  ├ Step 3.6  祝日/運休日を calendar_dates へ（holiday_* 指定時）
  ├ Step 4    shapes.txt 生成 (OSRM)
  ├ Step 4b   検証用マップ map_view.html 生成
  ├ Step 4c   GTFSビューア gtfs_viewer.html 生成（7タブ・常時）
  ├ Step 6    translations.txt 生成
  ├ Step 5    zip パッケージング
  ├ Step 7    GTFS Validator 検証           （validate=true 時）
  ├ Step 7b   GTFS-JP 拡張検証              （常時・Java 不要）
  ├ Step 7c   内部整合検証（抽出時刻↔stop_times）（extract_json 指定時）
  └ Step 7d   時刻アノマリ検出（OCR誤読の疑い） （extract_json 指定時）
```
