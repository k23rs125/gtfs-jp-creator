...
- **表の途中に方向見出し（「○○行き」）や便ヘッダ（「N便」/便番号/上り・下り）がある往復・
  上下レイアウトは、その行を境に縦分割して方向別ブロックにする**（同じ時刻列を共有して縦積みの
  時刻表で、別方向の便を連結する誤りを防ぐ）。方向見出しは `block.direction_hint` に保持しStep2の
  方向/行先割り当ての手がかりにする。実証: 太宰府まほろば号（市役所行き4便＋湯の谷2行き6便を分割）。
...
- **3.5f** `scripts/classify_coord_confidence.py` — **座標の信頼度分類（既定ON）**。各停留所の最終座標を
  補完源（公式再利用/手動/P11完全一致＝**確定**、P11あいまい/Nominatim/内挿/同名候補＝**要確認**、
  座標なし＝**未補完**）と**経路整合**（完全一致でも経路から外れれば要確認）で分類し，
  `output_dir/座標_信頼度.csv` に出力する。**官公庁提出など誤りが許されない用途で「推測座標を確定として
  黙って出さない」**ための層。要確認の座標は人手確認（アプリの地図確認等）に回す根拠になる。
