...
# バス停名でよく揺れる小書きカナ・異体字を代表字へ寄せ、表記違いでの取りこぼしを防ぐ。
# 例: 「夏井ケ浜」と「夏井ヶ浜」（ケ/ヶ）、「祗園崎」と「祇園崎」（祗/祇）は同一停留所。
# NFKC では統一されないため、ここで明示的に正規化する。地名で安全な対応のみを列挙する。
_VARIANT_MAP = str.maketrans({
    "ヶ": "ケ", "ヵ": "カ",                       # 小書きカナ
    "祗": "祇", "﨑": "崎", "髙": "高", "濵": "浜",  # 異体字（旧字・拡張漢字）
    "濱": "浜", "桒": "桑", "槗": "橋", "邊": "辺",
    "邉": "辺", "齋": "斎", "凉": "涼", "塚": "塚",
})
...
def normalize_name(name: str) -> str:
    """停留所名を比較用に正規化する（NFKC + 異体字/小書きカナ統一 + 空白統一）。"""
    if name is None:
        return ""
