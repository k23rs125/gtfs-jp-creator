...
def build_en_prompt(items: list[tuple[str, str]]) -> str:
    ...
    body.append("")
    body.append("# 英訳の方針")
    body.append("- 固有名詞は音訳（ローマ字）が基本")
    body.append("  例: 「リーパスプラザこが」→ \"Ripasu Plaza Koga\"")
    body.append("- 一般名詞部分は意訳")
    body.append("  例: 「古賀市役所」→ \"Koga City Hall\"、「中学校前」→ \"Junior High School\"")
    body.append("- 「JR」「駅」などの鉄道用語は英語慣用表現")
    body.append("  例: 「JR古賀駅東口」→ \"JR Koga Station East Entrance\"")
    body.append("- 括弧書きは省略可: 「(駅前広場)」→ 不要")
    body.append("")
    ...
    body.append("```json")
    body.append("{")
    body.append("  \"JR古賀駅東口\": \"JR Koga Station East Entrance\",")
    body.append("  \"リーパスプラザこが\": \"Ripasu Plaza Koga\",")
    body.append("  ...")
    body.append("}")
    body.append("```")
    body.append("")
