...
各項目は抽出元によって 3 つに分類する:
    ...
    [自動] 自動検出     PDF 抽出 + LLM で値が取れた項目
    [補完] 自動補完     生成時にスクリプトが自動で埋める項目（既定値・外部API）
    [要入力] 要入力       PDF に無く、ユーザー入力が必要な項目

利用者はこのサマリを見て、要入力（[要入力]）の項目を埋めてから生成に進む。
...
def build_summary(data: dict) -> tuple[str, int]:
    """確認サマリの Markdown 文字列と、要入力（[要入力] 必須）件数を返す。"""
    ...
    L.append("")
    L.append("凡例:  [自動] 自動検出（PDF+LLM）  "
             "[補完] 自動補完（生成時）  [要入力] 要入力（PDF外）")
    L.append("")
    ...
    L.append("────────── [!] 要入力サマリ ──────────")
    if missing:
        L.append(f"  [要入力] 要入力（GTFS-JP 推奨）: {missing} 件")
        L.append("     空欄のままでも生成は可能ですが、GTFS Validator で"
                 "警告が出ることがあります。")
    else:
        L.append("  [OK] 要入力の項目はありません。そのまま生成できます。")
    L.append("")
    L.append("────────── [OK] 次の操作 ──────────")
    L.append("  A) 値を編集する")
...
def main() -> int:
    ...
    print(f"[INFO] 要入力（[要入力] 必須）: {missing} 件", file=sys.stderr)
