...
```json
{
  ...
  "agency_jp": {
    "agency_official_name": "string|null (事業者の正式名称。PDF外の情報。例: '須恵町')",
    "agency_zip_number": "string|null (郵便番号 NNN-NNNN 形式。PDF外)",
    "agency_address": "string|null (事業者の住所。PDF外)",
    "agency_president_pos": "string|null (代表者の役職。任意。PDF外)",
    "agency_president_name": "string|null (代表者氏名。任意。PDF外)"
  },
  "office_jp": [
    {
      "office_id": "string (営業所ID。例: 'OFFICE01')",
      "office_name": "string (営業所名)",
  ...
  "calendar_dates": [
    {
      "service_id": "string (calendar の service_id を参照)",
      "date": "YYYYMMDD",
      "exception_type": 2,
      "comment": "string (任意。例: '年末年始運休'、'祝日')"
    }
  ],
  "fare_attributes": [
    {
      "fare_id": "string (例: 'F_150_R01' のような路線連動の命名)",
      "price": 150,
      "currency_type": "JPY",
      "payment_method": 0,
      "transfers": 0,
      "agency_id": "string (agency.agency_id を参照、任意)"
    }
  ],
  "fare_rules": [
    {
      "fare_id": "string (fare_attributes の fare_id を参照)",
      "route_id": "string (routes の route_id を参照)",
...
`agency_jp` / `office_jp` / `_meta.user_overrides` の扱い:

- `agency_jp` の各項目（正式名称・住所・郵便番号・代表者）は PDF にほぼ
  記載がない。**取れなければ null のままでよい**（条件確認画面でユーザーが入力する）。
- `office_jp` は営業所情報。PDF に記載が無ければ **空配列 `[]`** とする。
- `_meta.user_overrides` は条件確認画面でユーザーが上書きした値を
  `"table.field"` 形式のキーで格納する欄。Step 2 では空オブジェクト `{}` でよい。
...
11. **fare_attributes / fare_rules**: PDF に「運賃」「料金」の記載があれば抽出。
    - `fare_id` は `F_{price}_{route_id}` 形式（例: `F_150_R01`）
    - 路線別に異なる運賃なら fare_attributes・fare_rules を路線数分作る
    - `currency_type` は常に `"JPY"`
    - `payment_method` は通常 `0`（車内払い）、IC専用なら `1`
    - `transfers` は通常 `0`（乗継不可）
    - `route_id` は `routes` の `route_id` と一致させる
    - 大人/小学生など複数料金がある場合、**大人運賃のみ採用**（GTFS仕様で年齢別は標準外）
12. **calendar_dates**: PDF に「運休日」「年末年始運休」「祝日運休」の記載があれば抽出。
    - `service_id` は対応する `calendar` の `service_id` を参照
    - `date` は `YYYYMMDD` 形式
    - `exception_type` は `2`（運休）が基本、`1` は service 追加日
    - 「12/29〜1/3」のような期間表現は **個別日付に展開** して列挙
    - calendar 有効期間内（start_date〜end_date）の祝日も追記すると公式GTFS-JPに近い
    - `comment` 列は任意の説明（例: `"年末年始運休"`、`"月_海の日"`）
...
```json
{
  "agency": {
    ...
    "agency_url": null,
    "agency_phone": "092-932-1151"
  },
  "agency_jp": {
    "agency_official_name": null,
    "agency_zip_number": null,
    "agency_address": null,
    "agency_president_pos": null,
    "agency_president_name": null
  },
  "office_jp": [],
  "routes": [
    {
